...
const myColor = process.env.COLOR_MY_BRAND ? process.env.COLOR_MY_BRAND.toLowerCase() : "#007aff";
...
function generateCoreColors(hex) {
  ...
  const colors = {
    primary: hexFromArgb(Hct.from(hue, Math.max(chroma, 48), baseHct.tone).toInt()),
...
function generatePalette(hex) {
  ...
  const levels = [0, 4, 5, 6, 10, 12, 15, 17, 20, 24, 25, 30, 35, 40, 50, 60, 70, 80, 87, 90, 92, 94, 95, 96, 98, 99, 100];
  levels.forEach((level) => {
    tonalPalette[`level${level}`] = hexFromArgb(Hct.from(baseHct.hue, baseHct.chroma, level).toInt());
  });
